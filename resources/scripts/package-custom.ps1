...
function BuildInstallers() {
  ...
  foreach ($arch in $wixArchs) {
    ...
    dotnet wix build -arch $arch -ext WixToolset.UI.wixext -ext WixToolset.Util.wixext `
  ...
  dotnet wix build -arch "x64" -ext WixToolset.BootstrapperApplications.wixext `
  ...
  dotnet wix burn detach "./out/unsigned-installer-universal.exe" -engine "./out/engine.exe"
  # SignFiles @("out/engine.exe")

  dotnet wix burn reattach "./out/unsigned-installer-universal.exe" `
